...
local function drawFuelStrategySection(sim, cfg)
  ...
  if cfg.strategy.enabled then
    ...
    if not sim or not sim.isSessionStarted then
      ...
      if val ~= cfg.rules.minSpeedKmh then cfg.rules.minSpeedKmh = val; notifyChange() end
    end

    ui.newLine(2)
    ui.text("Tipo de Punição:")
    cfg.rules.penaltyType = cfg.rules.penaltyType or "slowdown"
    if ui.radioButton("Slowdown (desacelerar na pista)", cfg.rules.penaltyType == "slowdown") then
      cfg.rules.penaltyType = "slowdown"; notifyChange()
    end
    ui.sameLine(0, 10)
    if ui.radioButton("+5s adicionados ao tempo final", cfg.rules.penaltyType == "time5s") then
      cfg.rules.penaltyType = "time5s"; notifyChange()
    end
    ui.sameLine(0, 10)
    if ui.radioButton("Drive-Through (passagem pelo box)", cfg.rules.penaltyType == "dt") then
      cfg.rules.penaltyType = "dt"; notifyChange()
    end

    cfg.rules.slowdownSec = cfg.rules.slowdownSec or 5.0
    ui.setNextItemWidth(ui.windowWidth() - 60)
    local newSD = ui.slider("Tempo de Slowdown (s)", cfg.rules.slowdownSec, 1, 30, "%.0f s")
    if newSD ~= nil then
      local val = math.floor(clamp(newSD, 1, 30) + 0.5)
      if val ~= cfg.rules.slowdownSec then cfg.rules.slowdownSec = val; notifyChange() end
    end

    -- Penalty serving options (Mavil-style)
    ui.newLine(2)
    ui.separator()
    ui.text("Cumprimento de Penalidade no Box (Estilo Mavil):")

    cfg.rules.strictPit = (cfg.rules.strictPit == true)
    if ui.checkbox("Exigir 'isInPit' (mais estrito)", cfg.rules.strictPit) then
      cfg.rules.strictPit = not cfg.rules.strictPit; notifyChange()
    end

    cfg.rules.penaltyWaitTime = cfg.rules.penaltyWaitTime or 1.9
    ui.setNextItemWidth(ui.windowWidth() - 60)
    local newWT = ui.slider("Tempo de espera antes de cumprir (s)", cfg.rules.penaltyWaitTime, 0, 15, "%.1f s")
    if newWT ~= nil then
      local val = math.floor(clamp(newWT * 10, 0, 150) + 0.5) / 10
      if val ~= cfg.rules.penaltyWaitTime then cfg.rules.penaltyWaitTime = val; notifyChange() end
    end
    helpMarker("Simula o tempo do pit crew. 0 = cumprimento imediato.")

    cfg.rules.penaltyExtraTime = cfg.rules.penaltyExtraTime or 10
    ui.setNextItemWidth(ui.windowWidth() - 60)
    local newET = ui.slider("Tempo extra se sair do box sem cumprir (s)", cfg.rules.penaltyExtraTime, 0, 30, "%.0f s")
    if newET ~= nil then
      local val = math.floor(clamp(newET, 0, 30) + 0.5)
      if val ~= cfg.rules.penaltyExtraTime then cfg.rules.penaltyExtraTime = val; notifyChange() end
    end

    ui.newLine(2)
    cfg.rules.aiPenaltiesEnabled = (cfg.rules.aiPenaltiesEnabled ~= false)
    if ui.checkbox("IAs também recebem punições", cfg.rules.aiPenaltiesEnabled) then
      cfg.rules.aiPenaltiesEnabled = not cfg.rules.aiPenaltiesEnabled; notifyChange()
    end

    cfg.rules.aiServeInRace = (cfg.rules.aiServeInRace == true)
    if ui.checkbox("IAs cumprem slowdown nos boxes durante a corrida", cfg.rules.aiServeInRace) then
      cfg.rules.aiServeInRace = not cfg.rules.aiServeInRace; notifyChange()
    end

    -- Report generation
    ui.newLine(2)
    ui.separator()
    cfg.rules.writeReport = (cfg.rules.writeReport == true)
    if ui.checkbox("Gerar relatório TXT/CSV pós-corrida (Documents/Assetto Corsa/RaceFlow Reports)", cfg.rules.writeReport) then
      cfg.rules.writeReport = not cfg.rules.writeReport; notifyChange()
    end

    -- Player status
    if dirState then
      local playerWarnings = dirState.cutWarnings[0] or 0
      ui.newLine(2)
      ui.text(string.format("Suas Advertências: %d / %d", playerWarnings, cfg.rules.maxWarnings))

      local playerPen = dirState.activePenalties[0]
      if playerPen then
        if rgbm then
          ui.textColored(string.format("🛑 PUNIÇÃO ATIVA: %s! %.1f s restantes",
            playerPen.type:upper(), playerPen.remainingSec or 0), rgbm(1.0, 0.2, 0.2, 1.0))
        else
          ui.text(string.format("🛑 PUNIÇÃO ATIVA: %s! (%.1f s)", playerPen.type:upper(), playerPen.remainingSec or 0))
        end
      end
    end
  ...
  -- Feed de Rádio da Direção de Prova
  ui.newLine(4)
  ui.separator()
  ui.text("📻 Registro de Ocorrências (Direção de Prova)")
  if dirState and dirState.eventLogs and #dirState.eventLogs > 0 then
    for idx = 1, math.min(8, #dirState.eventLogs) do
      ui.textDisabled(dirState.eventLogs[idx])
    end
  else
    ui.textDisabled("Nenhuma ocorrência registrada nesta sessão.")
  end
end
